...
Todo `.kicker` do template só fica na cor de destaque (amarelo/accent) se tiver a classe `is-solution` junto (`.kicker.is-solution{color:var(--accent-text)}`). Sem essa classe, o CSS cai no padrão cinza (`--ink-muted`). `.kicker.is-problem` fica vermelho (`--danger-text`), reservado só pra slides de Observation/problema.

**Regra fixa: todo kicker que não é um "Observation" (problema) leva `is-solution`.** Isso vale pros seguintes kickers fixos do template — sempre gere com a classe, nunca deixe cinza:
...
- `THE BIG PICTURE`
- `NEXT STEPS`
- Kickers `OBSERVATION N` já levam `is-solution` (nunca mudou)
...
1. `"What's the issue with '[bottleneck]'?"` → vira o parágrafo do slide **Observation**
2. `"What's the fix for '[bottleneck]'?"` → vira o parágrafo do slide **Recommendation**
...
**6a. Transition (slide fixo, sempre entre o último Recommendation e a primeira Opportunity — não pergunte, é padrão):**
Antes de perguntar sobre opportunities, insira sempre este slide `slide-section` fixo (sem kicker, sem stat-row):
```html
<section class="slide slide-section">
  <div class="slide-inner">
    <h2 class="reveal">The Next Stage of Growth</h2>
    <p class="reveal" style="margin:20px auto 0">The recommendations you've seen improve performance today. The opportunities below are about building a stronger, more scalable business for tomorrow.</p>
  </div>
  <div class="slide-meta">...</div>
</section>
```
Copy fixa — não pergunte nem customize por cliente.
...
3. Delete os slides de exemplo (Observation/Recommendation 01–06, Opportunities 01–03) e reconstrua a sequência com a quantidade real de blocos que o usuário passou, seguindo a ordem exata que ele deu.
...
<section class="slide slide-split">
  <div class="slide-inner">
    <div class="split-media reveal">
      <div class="shot"><div class="shot-frame"><img src="/assets/market-context/market-growth.png" alt="Global online education market growth statistics"></div></div>
    </div>
...
<section class="slide slide-split flip">
  <div class="slide-inner">
    <div class="split-media reveal">
      <div class="shot"><div class="shot-frame"><img src="/assets/market-context/growth-leaks.png" alt="Diagram showing where growth actually leaks: slow funnels, poor follow-up, weak onboarding, disconnected automations, lost momentum before booking"></div></div>
    </div>
    <div class="split-text reveal">
...
5. Slide de intro: copy fixa, sempre a mesma estrutura (não pergunte — é padrão):
   ...
   - Mantém o `stat-row` abaixo (nº de observations / recommendations / opportunities).
6. Cada bottleneck vira 2 slides (Observation N / Recommendation N), na ordem dada.
7. Depois do último Recommendation, insira o slide fixo "Transition" (ver seção 6a) — sempre antes da primeira opportunity.
8. Cada opportunity vira 1 slide (`slide-section` com `icon-grid` ou `icon-circle` se tiver 5 itens visuais, senão sem ícones), com kicker "NEXT-LEVEL OPPORTUNITY N" (classe `is-solution`). Os `<img>` desses ícones apontam pra `/assets/icons/...` (caminho absoluto) — nunca copie o arquivo do ícone pra dentro de `images/` do cliente.
9. Depois da última opportunity, insira o slide fixo "The Big Picture" (ver seção 6c).
10. Slide final (regra fixa, não pergunte — nunca customize esse texto):
...
11. Atualize o `slide-meta` (número/total) de **cada** slide pro total real, em ordem sequencial sem pular nem repetir número. Slides fixos que **sempre** existem, independente do cliente: Capa, Market Context, Where Growth Leaks, Intro, Transition, The Big Picture, Closing — **7 slides fixos no total**. Some a isso 2 slides por bottleneck e 1 slide por opportunity: `total = 7 + (2 × nº de bottlenecks) + nº de opportunities`. Depois de montar todos os slides, **percorra o documento inteiro do primeiro ao último e confira que os números batem em sequência (01, 02, 03...) e que o total no segundo `<span>` é igual em todos os slides** — renumeração feita fora de ordem (ex. inserir um slide no meio sem recontar os que vêm depois) já causou bug de número duplicado/pulado antes.
12. **Checklist de imagens/screenshots (confira sempre antes de entregar)** — toda imagem de screenshot (facts, fixes, market-context) deve estar dentro de um `.shot-frame` pra não distorcer:
   - Estrutura: `<div class="shot">...<div class="shot-frame"><img src="..."></div></div>` (o `.shot-bar` de três bolinhas, quando presente, fica fora do `.shot-frame`, só o `<img>` entra dentro).
   - Nunca deixe um `<img>` solto direto dentro de `.shot` sem passar pelo `.shot-frame` — sem ele, imagens retrato/paisagem/tamanhos variados esticam ou cortam de forma inconsistente.
13. **Checklist do botão `.cta-btn` (confira sempre antes de entregar)** — já vem certo no template de referência, mas confira que nenhuma edição quebrou isso:
   ...
   - **O `:hover` e o `:active` precisam de `!important` no `transform` e no `box-shadow`** — sem isso, a regra `.slide.visible .reveal` (3 classes de especificidade) vence o `.cta-btn:hover` (1 classe + pseudo-classe) e a animação de hover praticamente não aparece (só o box-shadow muda, quase imperceptível).
   - O botão tem um pulse contínuo e sutil no glow (`@keyframes cta-pulse`, 2.4s, alternando o `box-shadow`), que pausa (`animation-play-state:paused`) no `:hover`. Nunca remova esse `animation` ao editar o botão.
14. **Checklist dos botões de navegação ↓↑ (confira sempre antes de entregar)** — o `.kbd-hint` precisa de dois `id`s pro JS funcionar:
   - No HTML: `<kbd id="navDown" style="cursor:pointer">↓</kbd><kbd id="navUp" style="cursor:pointer">↑</kbd>`.
   - No `<script>`: funções `goNext()`/`goPrev()` + `document.getElementById('navDown')`/`'navUp'` com `addEventListener('click', ...)`. Sem os `id`s no HTML, o `getElementById` retorna `null` e o clique não faz nada.
15. **Checklist de kickers (confira sempre antes de entregar)** — todo `.kicker` que não seja um `OBSERVATION N` precisa da classe `is-solution` (ver "Regra de branding" acima). Um kicker sem `is-solution` e sem `is-problem` é um bug — vai renderizar cinza em vez da cor do branding.
