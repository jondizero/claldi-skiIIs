...
**7. Fechamento:**
> "Closing CTA text? (or 'default' to reuse: 'Does This Make Sense?' / 'If so, just click the button and shoot me a message on IG.')"

Depois de tudo respondido, confirme um resumo curto (nome do cliente, nº de bottlenecks, nº de opportunities) antes de gerar o HTML final.

## Como montar o HTML

1. Copie `references/gar-capital-example.html` pra um novo arquivo de trabalho.
2. Troque `--green:#00ff94` pela `Brand Color` informada (é a única variável CSS que muda — todo o resto do design system fica igual).
...
4. Slide de capa: nome do cliente + business name (se houver) no lugar de "GAR Capital", logo se houver.
5. Slide de intro: personalize com o `Client Name` no lugar de "Carlos", mantendo a estrutura de frase ("[Nome], I checked SOME parts of your info setup...").
6. Cada bottleneck vira 2 slides (Fact N / Fix N), na ordem dada.
7. Cada opportunity vira 1 slide (`slide-section` com `icon-grid` ou `icon-circle` se tiver 5 itens visuais, senão sem ícones). Os `<img>` desses ícones apontam pra `/assets/icons/...` (caminho absoluto) — nunca copie o arquivo do ícone pra dentro de `images/` do cliente.
8. Slide final: CTA fixo do Instagram, texto customizado se pedido.
9. Atualize o `slide-meta` (número/total) de cada slide pro total real.
