...
12. **Checklist de kickers (confira sempre antes de entregar)** — todo `.kicker` que não seja um `OBSERVATION N` precisa da classe `is-solution` (ver "Regra de branding" acima). Um kicker sem `is-solution` e sem `is-problem` é um bug — vai renderizar cinza em vez da cor do branding.
